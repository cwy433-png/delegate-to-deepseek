...
Keep API credentials in `DEEPSEEK_API_KEY` or the macOS Keychain item `codex-deepseek-api`. Exclude `.env`, private keys, credential stores, and unrelated personal files from delegated tasks unless the user explicitly places them in scope.
